...
namespace Server
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var isConnect = false;
            Server.Start(3000000, 2020);

            TcpClient tcp = new TcpClient();
            tcp.ReceiveBufferSize = 2048;
            tcp.SendBufferSize = 2048;

            tcp.BeginConnect("127.0.0.1", 2020, (result) =>
            {
            ...
            while (true)
            {
                if (isConnect)
                {
                    string msg = "Hello Time: " + GetTimeStamps();
                    var data = Encoding.ASCII.GetBytes(msg);
                    tcp.Client.Send(data, data.Length, SocketFlags.None);
                    break;
                }
            }
        ...
        public class Tcp
        {
            ...
            private void ReceiveCallBack(IAsyncResult result)
            {
                try
                {
                    ...
                    string msg = Encoding.ASCII.GetString(receiveBuffer, 0, byteLength);
                    Console.WriteLine(msg);
                    Console.WriteLine("Cur Time " + Program.GetTimeStamps());
